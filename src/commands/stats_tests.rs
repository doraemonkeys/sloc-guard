...
fn format_stats_output_text() {
    ...
    let output = format_stats_output(OutputFormat::Text, &stats, ColorMode::Never).unwrap();
...
fn format_stats_output_json() {
    let stats = ProjectStatistics::new(vec![]);
    let output = format_stats_output(OutputFormat::Json, &stats, ColorMode::Never).unwrap();
    assert!(output.contains("summary"));
}
...
fn format_stats_output_sarif_not_implemented() {
    let stats = ProjectStatistics::new(vec![]);
    let result = format_stats_output(OutputFormat::Sarif, &stats, ColorMode::Never);
...
fn format_stats_output_markdown_works() {
    let stats = ProjectStatistics::new(vec![]);
    let result = format_stats_output(OutputFormat::Markdown, &stats, ColorMode::Never);
    assert!(result.is_ok());
