...
fn override_does_not_match_partial_filename() {
    ...
    let result = checker.check(Path::new("src/my_parser.rs"), &stats, None);
...
fn override_matches_exact_filename() {
    ...
    let result = checker.check(Path::new("src/parser.rs"), &stats, None);
...
fn override_matches_full_path() {
    ...
    let result = checker.check(Path::new("src/legacy/parser.rs"), &stats, None);
    assert!(result.is_passed());
    assert_eq!(result.limit(), 800);

    // "other/src/legacy/parser.rs" should also match (ends with)
    let result2 = checker.check(Path::new("other/src/legacy/parser.rs"), &stats, None);
    assert!(result2.is_passed());
    assert_eq!(result2.limit(), 800);

    // "legacy/parser.rs" should NOT match (missing "src" component)
    let result3 = checker.check(Path::new("legacy/parser.rs"), &stats, None);
